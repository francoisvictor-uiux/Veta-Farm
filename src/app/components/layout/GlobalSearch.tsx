...
import {
  Search, X, ShoppingCart, Tag, Landmark,
...
interface Result {
  id: string
  category: 'purchase' | 'sale' | 'cashier' | 'hr'
  title: string
  sub: string
  path: string
  value?: string
...
function Highlighted({ text, query }: { text: string; query: string }) {
  ...
  return (
    <>
      {text.slice(0, idx)}
      <mark className="bg-yellow-100 text-yellow-800 rounded px-0.5 not-italic">{text.slice(idx, idx + query.length)}</mark>
...
export function GlobalSearch({ placeholder = 'بحث سريع...', className = '' }: Props) {
  ...
  const results = useMemo((): Result[] => {
    ...
    purchaseOrders
      ...
      .slice(0, 3).forEach(o => out.push({
        id: o.id, category: 'purchase', path: '/purchasing',
        title: o.orderNumber, value: fmt(o.totalAmount),
        sub: `${o.supplierName} · ${o.items.map(i => i.name).join('، ')}`,
      }))
    saleOrders
      .filter(o => o.orderNumber.toLowerCase().includes(q) || o.customerName.includes(q) || o.items.some(it => it.name.includes(q)))
      .slice(0, 3).forEach(o => out.push({
        id: o.id, category: 'sale', path: '/sales',
        title: o.orderNumber, value: fmt(o.totalAmount),
        sub: `${o.customerName} · ${o.items.map(i => i.name).join('، ')}`,
      }))
    transactions
      ...
      .slice(0, 3).forEach(t => out.push({
        id: t.id, category: 'cashier', path: '/cashier',
        title: t.txNumber, value: fmt(t.amount), sub: t.description,
      }))
    employees
      .filter(e => e.name.includes(q) || e.employeeNumber.toLowerCase().includes(q) || e.department.includes(q) || e.jobTitle.includes(q))
      .slice(0, 3).forEach(e => out.push({
        id: e.id, category: 'hr', path: '/attendance',
        title: e.name, sub: `${e.employeeNumber} · ${e.jobTitle} · ${e.department}`,
      }))
    return out
  }, [q])
  ...
  return (
    <div ref={wrapRef} className={`relative ${className}`} dir="rtl">
      <div className={`flex items-center gap-2 h-9 px-3 rounded-[10px] border transition-all duration-150 ${
        focused ? 'bg-white border-[#1a6b3c]/40 ring-2 ring-[#1a6b3c]/15 shadow-sm' : 'bg-neutral-100 border-neutral-200'
      }`}>
        ...
        <input
          ...
          className="flex-1 bg-transparent outline-none font-cairo text-[13px] text-neutral-700 placeholder:text-neutral-400 min-w-0"
          dir="rtl"
        />
        {query && (
                  ...
                  <div key={cat}>
                    ...
                    {items.map(r => (
                      <button key={r.id} onClick={() => go(r.path)}
                        className="w-full flex items-center gap-3 px-4 py-3 border-b border-neutral-50 hover:bg-neutral-50 transition-colors text-right group">
                        <div className={`w-8 h-8 rounded-lg ${cfg.bg} flex items-center justify-center shrink-0`}>
                          <CatIcon size={13} className={cfg.color} />
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="font-cairo font-semibold text-[12px] text-neutral-800 truncate">
                            <Highlighted text={r.title} query={query} />
                          </p>
                          <p className="font-cairo text-[11px] text-neutral-400 truncate">
                            <Highlighted text={r.sub} query={query} />
                          </p>
                        </div>
